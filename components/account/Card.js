...
export default function Card({img,title,price}){
    return (
        <Box pb='30px' sx={{boxShadow: "rgba(100, 100, 111, 0.2) 0px 7px 29px 0px",borderRadius:"10px",diplay:'flex',flexDirection:"column",textAlign:"center",bgcolor:'white'}}>
            <img src={img} height='220' width=""/>
            <Box sx={{display:"flex",flexDirection:"column",gap:"30px"}}>
                <Box>{title}</Box>
                <Box><Typography variant={'h5'} component={'h5'}>{'\u20B9'}{price}</Typography></Box>
            </Box>
            <Box pl='50px' sx={{display:"flex",gap:'20px'}}>
